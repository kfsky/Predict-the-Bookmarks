...
def preprocess(input_df):
    ...
    # 文章の数値データの置換（０に置き換える）
    output_df["story"] = output_df["story"].map(replace_number_to_zero)
...
def main():
    ...
    # それぞれの予測値平均を算出する
    train["pred_mean_00"] = (train["pred_2019_00"] + train["pred_all_00"])/2
    train["pred_mean_01"] = (train["pred_2019_01"] + train["pred_all_01"]) / 2
    train["pred_mean_02"] = (train["pred_2019_02"] + train["pred_all_02"]) / 2
    train["pred_mean_03"] = (train["pred_2019_03"] + train["pred_all_03"]) / 2
    train["pred_mean_04"] = (train["pred_2019_04"] + train["pred_all_04"]) / 2

    test["pred_mean_00"] = (test["pred_2019_00"] + test["pred_all_00"]) / 2
    test["pred_mean_01"] = (test["pred_2019_01"] + test["pred_all_01"]) / 2
    test["pred_mean_02"] = (test["pred_2019_02"] + test["pred_all_02"]) / 2
    test["pred_mean_03"] = (test["pred_2019_03"] + test["pred_all_03"]) / 2
    test["pred_mean_04"] = (test["pred_2019_04"] + test["pred_all_04"]) / 2
