...
def preprocess(input_df):
    ...
    # タイトルとstory結合
    output_df["title_story"] = output_df["title"] + output_df["story"]

    # novel_type + genre
    output_df["genre_novels"] = output_df["genre"].astype(str) + "_" + output_df["novel_type"].astype(str)
...
def set_stopwords():
    ...
    slothlib_stopwords = joblib.load(os.path.join(config.INPUT, "stopwords.jb"))
...
def mecab_tokenizer(s: str):
    ...
    return [
        ...
        if result.split('\t')[1].split(',')[0] not in ['助詞', '助動詞', '接続詞', '動詞', '記号']
        if result.split("\t")[0] not in stopwords
    ]

def mecab_tokenizer_title_keyword(s: str):
    tagger = MeCab.Tagger("../usr/local/lib/mecab/dic/mecab-ipadic-neologd")
    parse_result = tagger.parse(s)
    return [
        result.split("\t")[0]
...
class TfidfBlock(BaseBlock):
    ...
    def fit(self, input_df, y=None):
        ...
        if self.decomposition == "svd":
            if self.column == "story" or self.column == "title_story":
                ...
                self.pipeline_ = Pipeline([
                    ("tfidf", TfidfVectorizer(tokenizer=mecab_tokenizer_title_keyword, max_features=10000)),
                    ("svd", TruncatedSVD(n_components=50, random_state=71)),
                ])
    ...
    def fit(self, input_df, y=None):
        ...
        if self.decomposition == "svd":
            if self.column == "story" or self.column == "title_story":
                ...
                self.pipeline_ = Pipeline([
                    ("CountVectorizer", CountVectorizer(tokenizer=mecab_tokenizer_title_keyword, max_features=10000)),
                    ("BM25Transformer", BM25Transformer()),
                    ("svd", TruncatedSVD(n_components=50, random_state=71)),
                ])
...
class TargetEncodingBlock(BaseBlock):
    ...
    def fit(self,input_df):
        ...
        vc = df[self.column].value_counts()
        idx = vc[vc>=10].index
        df = df[df[self.column].isin(idx)]

        #df = df.merge(input_df[[self.column,'fav_novel_cnt_bin']],on=self.column,how='left')

        self.te = category_encoders.TargetEncoder(smoothing=0.1).fit(df[self.column], df['fav_novel_cnt_bin'])

        return self.transform(input_df)

    def transform(self,input_df):
      output_df = pd.DataFrame()
      output_df[f"TE_{self.column}"] = self.te.transform(input_df[self.column])[self.column]
      return output_df
...
def main():
    ...
    # 前の結果を追加
    _oof_043 = Util.load("../output/exp043/preds/oof_CLASS.pkl")
    _oof_043_df = pd.DataFrame(_oof_043)

    _pred_043 = Util.load("../output/exp043/preds/preds_CLASS.pkl")
    _pred_043_df = pd.DataFrame(_pred_043)

    oof_columns3 = [f"pred_043_{i:02d}" for i in range(5)]
    _oof_043_df.columns = oof_columns3
    _pred_043_df.columns = oof_columns3

    # 変更
    train = train.join(_oof_043_df)
    test = test.join(_pred_043_df)

    # 結合データ作成
    whole_df = pd.concat([train, test], axis=0)

    process_blocks = [
        *[TargetEncodingBlock(c) for c in [
            "writer",
            "genre",
            "biggenre",
            "genre_novels"
