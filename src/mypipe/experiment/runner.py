...
class Runner:
    def __init__(self, config, run_params, model_params, fit_params, model, features, use_mlflow=False):
        ...
        self.logger.info(f'exp_name : {config.EXP_NAME}')
    ...
    def run_train_cv(self):
        ...
        train_y = self.load_y_train()  # y_true
    ...
    def run_predict_cv(self) -> None:
        ...
        for seed in self.seeds:
            ...
            for i_fold in range(self.folds):
                ...
                model = self.build_model(seed, i_fold)
                model.load_model()
                pred = model.predict_proba(test_x.values)
                preds.append(pred)
            preds = np.mean(preds, axis=0)
            #preds = mode(preds)[0][0]
            preds_seeds.append(preds)

        preds = np.mean(preds_seeds, axis=0)
        #pred_label = np.argmax(preds, axis=1)
        # preds = mode(preds_seeds)[0][0]
