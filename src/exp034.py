...
def get_umap_title(input_df):
    col_prefix = "title"
    if len(input_df) == 28861:
        titles = np.load("../add_data/train_title_roberta_2020-21.npy")
    else:
        titles = np.load("../add_data/test_title_roberta_2020-21.npy")

    title_df = pd.DataFrame(titles)
    um = umap.UMAP(n_components=50, random_state=71)
    _umap = um.fit_transform(title_df)
    umap_df = pd.DataFrame(_umap, columns=[f'{col_prefix}_umap{c}' for c in range(1, 51)])
    return umap_df
...
def get_umap_story(input_df):
    col_prefix = "story"
    if len(input_df) == 28861:
        stories = np.load("../add_data/train_story_roberta_2020-21.npy")
    else:
        stories = np.load("../add_data/test_story_roberta_2020-21.npy")
...
def main():
    ...
    oof_columns = [f"pred_{i:02d}" for i in range(5)]
    _oof_df.columns = oof_columns
    _pred_df.columns = oof_columns

    train = train.join(_oof_df)
