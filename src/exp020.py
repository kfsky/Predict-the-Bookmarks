...
class TargetEncodingBlock(BaseBlock):
    def __init__(self, target, column, k=100, impute=True):
        self.target = target
        self.column = column
        self.k = k
        self.impute = impute
    ...
    def fit(self, input_df, y=None):
        ...
        if self.impute:
            self.target_mean = input_df[self.target].mean()

        k = self.k
        n_i = input_df.groupby(self.column).count()[self.target]

        lambda_n_i = self.custom_sigmoid(n_i, k)
        uni_map = input_df.groupby(self.column).mean()[self.target]

        self.target_map[self.column] = lambda_n_i * input_df.loc[:, self.target].mean() + (1 - lambda_n_i) * uni_map

        return self.transform(input_df)

    def transform(self, input_df):
        output_df = input_df.copy()
        output_df.loc[:, self.column] = output_df.loc[:, self.column].map(self.target_map[self.column])

        if self.impute:
            output_df.loc[:, self.column] = np.where(output_df.loc[:, self.column].isnull(),
                                                     self.target_mean, output_df.loc[:, self.column])

        return output_df[[self.column]].add_prefix("TE_")
...
def main():
    ...
    process_blocks = [
        ...
        TargetEncodingBlock(target="fav_novel_cnt_bin", column="writer", k=100),
